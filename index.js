...
app.post('/characters', (req, res) => {
  ...
  if (!name || !type || health === undefined || damage === undefined) {
    return res
      .status(400)
      .json({ message: 'Missing required fields: name, type, health, damage' });
  }
...
app.delete('/characters/:id', (req, res) => {
  ...
  res
    .status(200)
    .json({ message: `Character with id ${id} deleted successfully` });
});
...
app.get('/characters/type/:type', (req, res) => {
  ...
  if (type !== 'plant' && type !== 'zombie') {
    return res
      .status(400)
      .json({ message: "Invalid type. Must be 'plant' or 'zombie'." });
  }
...
app.get('/random', (req, res) => {
  if (characters.length === 0) {
    return res
      .status(404)
      .json({ message: 'No characters available to choose from.' });
  }
...
app.get('/characters/strongest', (req, res) => {
  ...
  const strongest = characters.reduce((prev, current) =>
